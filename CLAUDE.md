...
### Conventions & gotchas
- **Persistence is MariaDB** (`migrations/`, `src/server/db.ts`) — do not introduce Postgres-specific SQL. All SQL lives in `src/server/repo/` and is parameterized. `src/lib/seed.ts` is a fixture used by `db:seed` and the e2e reset endpoint.
- **Hierarchy is project-based** (Phase 2): `items.project_id` → `projects`; `project_teams` (a project can be owned by MULTIPLE teams) and `team_members` (a team contains multiple users) are both M:N. `GET /api/structure` serves the whole read-side; the Navigator groups items by project and lists teams; selecting a team opens the scrum **team space** (`TeamSpace.tsx`: sprint board over the team's projects' work items, ranked backlog of sprint-less WIs, committed/done points). `item.area` is a display label only. Spawned iteration children inherit the parent's `project_id`.
- **Admin writes** (Phase 3, PM-only via `requirePM`): `POST /api/projects`, `POST /api/teams`, `POST /api/teams/:id/members|projects` (`{op:"add"|"remove"}`), `PATCH /api/items/:id/project`. Writes live in `repo/structure.ts` and return `{ok:false,error}` for expectable conflicts (mapped to 422). Item→project assignment is a direct column update, NOT an event — project membership is metadata, not lifecycle. The e2e reset endpoint restores the seed hierarchy (extra runtime-created teams/projects are wiped).
