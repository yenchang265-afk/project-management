/* =========================================================================
   ...
   ========================================================================= */
import type { EventType, Item, PdlcEvent, Role, Stakeholder } from "./engine";
...
const QAROLE: Stakeholder = { role: "QA Lead", name: QA };
const SECROLE: Stakeholder = { role: "Security Reviewer", name: SEC };
// pm = Product Manager, tl = Tech Lead; Eng Manager + Designer are constant for this org
const roster = (pm: string, tl: string, extra?: Stakeholder[]): Stakeholder[] => [
...
type EventSpec = [number, EventType, string, Role, Partial<PdlcEvent>?];

export interface SeedData {
  ITEMS: Item[];
  people: { PM: string; DEV: string; PM2: string; DEV2: string };
  org: { key: string; name: string; sub: string };
  orgs: string[];
  groups: { key: string; label: string; teams: string[] }[];
}

/* Built lazily with a caller-supplied "now" so timestamps are stable per session
   (the prototype computed them once at script load). */
export function buildSeed(now: number): SeedData {
  let n = 0;
  // ageDays ago -> timestamp
  const at = (d: number) => now - d * DAY;

  function E(itemId: string, ageDays: number, type: EventType, actor: string, role: Role, payload?: Partial<PdlcEvent>): PdlcEvent {
    return { id: "seed-" + ++n, item: itemId, type, actor, role, ts: at(ageDays), ...(payload || {}) };
  }

  function item(meta: Omit<Item, "events">, eventSpecs: EventSpec[]): Item {
    return {
      ...meta,
